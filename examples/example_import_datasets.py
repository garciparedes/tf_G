...
def compose_from_name(name: str, index_decrement: bool) -> np.ndarray:
  return tfgraph.DataSets.compose_from_path(name_to_default_path(name),
                                         index_decrement)
...
def main():
  with tf.Session() as sess:
    print(tfgraph.GraphConstructor.from_edges(sess, "G_followers", followers()))

    print(tfgraph.GraphConstructor.from_edges(sess, "G_p2p", p2p_gnutella08()))

    print(tfgraph.GraphConstructor.from_edges(sess, "G_wiki", wiki_vote()))
