  ...
  \label{chapter:streaming}
    ...
    \label{sec:streaming_model}
      ...
      \label{sec:streaming_formalism}
        ...
        \paragraph{}
        Sea $a_1 ,a_2 ,... ,a_t,... $ un flujo de datos de entrada (\emph{Input Stream}), de tal manera que cada elemento debe presentar un orden de llegada secuencial respecto de $t \in \mathbb{N}$. Esto puede traducirse de la como: el elemento siguiente a la llegada de $a_{t-1}$ debe ser $a_{t}$ y, por inducción, el próximo será $a_{t+1}$.

        \paragraph{}
        Lo siguiente para describir el formalismo es añadir la función $\boldsymbol{A}_t:[1...N] \rightarrow \mathbb{R}^2$, la cual tiene distintas interpretaciones dependientes del \emph{Modelo en Streaming} con el cual se esté tratando. Sin embargo algo común a todos ellos es la variable $t$, que se corresponde con el valor de la función en dicho instante de tiempo. Por motivos de claridad, en los casos en que nos estemos refiriendo al instante actual, dicha variable será eliminada de la notación.

      \subsection{Modelo de Serie Temporal}
      \label{sec:streaming_time_series}

        \paragraph{}
        El \emph{Modelo de Serie Temporal} o \emph{Time Series Model} es aquel en el cual se define la función $\boldsymbol{A}$ tal y como se describe en la ecuación \eqref{eq:streaming_time_series}. De manera textual, esto puede traducirse diciendo que la función $\boldsymbol{A}$ representa una estructura de datos que almacena el valor de todos los elementos presentados en la entrada hasta el instante de tiempo $t$, es decir, actúa como un historial. Un ejemplo de este modelo son los valores en bolsa que toma una determinada empresa a lo largo del tiempo, y la función $\boldsymbol{A}$ es un historial del mismo.
