...
\begin{document}
  ...
  \label{chap:graphs}
    ...
    \label{sec:graphs_intro}
      ...
      \paragraph{}
      En las últimas décadas se han dedicado grandes esfuerzos en el estudio de sucesos basados en interelaciones entre elementos. Esto puede entenderse como una red de interconexiones modeladas como flechas sobre un conjunto de puntos. Una gran cantidad de situaciones de la vida cotidiana puede ser representada de esta manera, mediante la cual se consigue un formalismo matemático enriquecedor sobre el cual resolver distintos problemas que surgen sobre dichas situaciones.

      \paragraph{}
      Estas redes de interconexiones se pueden apreciar en un gran número de situaciones, como las ciudades y las carreteras que conectan unas con otras, las personas y las amistades entre si, los teléfonos y las llamadas de unos a otros o las páginas web de internet y los enlaces para navegar de unas a otras. El estudio de estas situaciones es de gran interés en las sociedades modernas, permitiendo una mejora del rendimiento a partir de la extracción de información poco obvia mediante distintas técnicas lo cual conlleva reducción de costes o aumentos del grado de satisfacción de los usuarios.

      \paragraph{}
      Sin embargo, utilizar un lenguaje de representación más enriquecedor conlleva distintos sobrecostes asociados respecto de otros modelos más simples, lo cual requiere de técnicas sofisticadas para tratar de hacer frente a la resolución de los problemas que se pretende resolver. Además, el crecimiento exponencial en la infraestructura tecnológica ha conllevado un gran aumento en la complejidad sobre problemas de redes. Algunos ejemplos destacados en el ámbito de internet se dan el protocolo IPv6, que cuenta con $2^128$ direcciones posibles entre las que poder comunicarse, o el caso de la red social \emph{Facebook}, que cuenta con 1 trillón de relaciones de amistad tal y como se indica en \emph{One trillion edges: Graph processing at facebook-scale} \cite{ching2015one}.

      \paragraph{}
      Conforme el tamaño de las redes aumentan una estrategia razonable es utilizar soluciones aproximadas para la resolución de los problemas que se plantean sobre ellas, a través de los cuales se pretende encontrar una solución cercana a la exacta (con una desviación máxima de $\epsilon$ que se cumpla con probabilidad $\delta$) que otorgue como ventaja una reducción significativa tanto desde la perspectiva del coste temporal como del espacial en la resolución del problema.

      \paragraph{}
      En este capítulo se pretende realizar una descripción acerca de las distintas alternativas posibles para la resolución a problemas sobre redes de tamaño masivo utilizando técnicas aproximadas. Por tanto, lo primero que se ha realizado una descripción formal sobre el modelo de representación de grafos en la sección \ref{sec:graph_formalism}. A continuación se ha descrito un modelo sobre el cual diseñar algoritmos que resuelvan problemas aplicados a grafos tratando de aprovechar al máximo el espacio en memoria (que se considera limitada) y reduciendo el número de accesos sobre el espacio de almacenamiento mediante el \emph{modelo en semi-streaming} en la sección \ref{sec:semi_streaming_model}. El siguiente tema que se trata en este capítulo son técnicas que tratan de reducir la complejidad de una red de tamaño masivo mediante la eliminación de relaciones entre sus puntos, por lo que se ha hablado de \emph{Spanners} y \emph{Sparsifiers} en la sección \ref{sec:spanners_sparsifiers}. Después, se ha realizado una breve descripción acerca de distintos problemas bien conocidos para los cuales se han encontrados soluciones sobre el \emph{modelo en semi-streaming} en la sección \ref{sec:graph_problems}. Finalmente se ha muestran distintas conclusiones en la sección \ref{sec:graph_conclusions}.
    ...
    \label{sec:graph_formalism}
      ...
      \label{sec:representation_methods}
        ...
        \label{sec:laplacian_matrix}
          ...
          \paragraph{}
          La \emph{matriz laplaciana} consiste en una estrategia de representación de grafos que cumple importantes propiedades a partir de las cuales se facilita en gran medida la resolución de un gran número de problemas, entre los que se encuentran \emph{árboles de recubrimiento} (sección \ref{sec:minimum_spanning_tree}), aunque también se utiliza en la estimación de la distribución de probabilidad de \emph{paseos aleatorios} (sección \ref{sec:random_walks_overview}).

          \paragraph{}
          El método de construcción de la \emph{matriz laplaciana} se indica en la ecuación \eqref{eq:laplacian_matrix}, el cual genera una matriz $L$ de tamaño $n * n$ que aporta información del grafo subyacente. La diagonal de la matriz laplaciana contiene el grado del vértice representado en dicha posición, mientras que el resto de las celdas se construyen colocando el valor $-1$ cuando existe un arista entre el vértice $v_i$ y el vértice $v_j$ ( $(v_i,v_j) \in E$ ) y $0$ cuando no existe.
